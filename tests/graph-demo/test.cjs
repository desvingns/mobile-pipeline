...
function open(hash='',options={}){
  ...
  if(options.theme)w.localStorage.setItem('mp-demo-theme',options.theme);
  ...
  for(const f of ['assets/theme.js','story/scenario.js','story/timing.js','assets/demo.js'])w.eval(fs.readFileSync(path.join(graph,f),'utf8'));
...
t=open('#simple/research',{theme:'paper'});assert.equal(t.doc.documentElement.dataset.theme,'paper');assert.equal(t.doc.querySelector('[data-theme-choice=paper]').getAttribute('aria-pressed'),'true');t.click('[data-theme-choice=light]');assert.equal(t.w.localStorage.getItem('mp-demo-theme'),'light');assert.equal(t.w.location.hash,'#simple/research');assert.equal(t.doc.querySelector('meta[name=theme-color]').content,'#edf0e9');
t.click('[data-mode=details]');let received=null;const ef=t.doc.getElementById('expert-frame');ef.contentWindow.postMessage=message=>{received=message;};t.doc.dispatchEvent(new t.w.Event('DOMContentLoaded'));ef.dispatchEvent(new t.w.Event('load'));assert.equal(received.theme,'light');t.click('[data-theme-choice=dark]');assert.equal(received.theme,'dark');assert.equal(t.doc.querySelectorAll('[data-theme-choice][aria-pressed=true]').length,1);t.close();
t=open('#simple/checks',{file:true,blockStorage:true});t.click('[data-theme-choice=paper]');assert.equal(t.doc.documentElement.dataset.theme,'paper');t.click('[data-mode=presentation]');assert.equal(t.doc.documentElement.dataset.theme,'paper');t.close();
t=open('#simple/idea',{theme:'invalid'});assert.equal(t.doc.documentElement.dataset.theme,'dark');t.close();
...
const themeParent=new JSDOM('<!doctype html>').window;
Object.defineProperty(ew,'parent',{value:themeParent});
ew.eval(fs.readFileSync(path.join(graph,'assets/theme.js'),'utf8'));
ew.dispatchEvent(new ew.MessageEvent('message',{source:themeParent,data:{type:'mp-graph-theme',theme:'paper'}}));
assert.equal(ed.documentElement.dataset.theme,'paper');
ew.dispatchEvent(new ew.MessageEvent('message',{source:ew,data:{type:'mp-graph-theme',theme:'dark'}}));
assert.equal(ed.documentElement.dataset.theme,'paper','foreign theme messages must be ignored');
themeParent.close();
...
console.log('PASS: story refs, parallel join, sequential execution, cancellation, 4 modes, legacy links, dialogs, phone, chapters, file URLs, reduced motion, keyboard, expert search/zoom, 3 themes, persistence, frame sync and local assets.');
