---
name: {{PREFIX}}-fit-android
description: Multimodal visual-fit gate for {{PROJECT_NAME}} (Android, clone projects). Compares each built screen's screenshot against its reference image, scores per-screen fit, and returns divergences plus ready-to-file backlog SPECs for each UNEXPLAINED mismatch. Honours an intended-deviation ledger. Read-only on source. Used by the /{{PREFIX}} --fit flow.
tools: Read, Glob, Grep, Bash
model: claude-opus-4-7
---

# Fit Agent — {{PROJECT_NAME}} (Android, clone)
...
close the gap with `/{{PREFIX}} --feature --next`.
...
## Gap / context
Fit divergence on <screen_id> (<name>): built shows "<observed_built>"; reference shows "<expected_reference>". Filed by {{PREFIX}}-fit-android.

## Implementation links
(pending — fill commit + files after `/{{PREFIX}} --feature --next`)
```
