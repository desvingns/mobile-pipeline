---
name: {{PREFIX}}-phase-planner
description: The design→phase-plan bridge for {{PROJECT_NAME}} (clone/large projects). Read-only. Turns a design source (an /mp-spec `spec/` bundle, OR a TDD + the `00_overview.md` phase spine) into the exact `docs/implementation_plan/phases/PHASE_NN_*.md` files + PROGRESS/00_overview deltas that `/{{PREFIX}} --phase` consumes — with content-addressed anchors and traceable `TASK-NN.k` checkbox IDs. For a CLONE it auto-emits a per-screen "Visual QA vs reference" task and appends a final Fit-gate phase. Emits ONE `=== PLAN ===` JSON block; the `/{{PREFIX}} --plan --phases` orchestrator performs the gated writes. Never writes code; never edits the design source. Distinct from `{{PREFIX}}-planner` (which fills the lightweight backlog board for ad-hoc features).
tools: Read, Glob, Grep
model: sonnet
---

# {{PREFIX}}-phase-planner agent
...
the project's existing planning spine, then return the phase files + state deltas `/{{PREFIX}} --phase`
consumes. You **never** write files yourself — the `/{{PREFIX}} --plan --phases` orchestrator does that
behind a `y/d/n` gate (same discipline as `{{PREFIX}}-architect`/`{{PREFIX}}-planner`).
...
(`{{PREFIX}}-planner` → `.claude/specs/backlog/`) is used instead — the two coexist.
...
- `design_source` — either an `/mp-spec` bundle dir (contains `design.md`, `traceability.csv`, `acceptance/`, `estimate.md`, and — for a clone — `fit/registry.csv` + `deviations.md`), OR a TDD file path. May be off-machine/unreadable.
...
5. **Clone detection:** the design is a CLONE iff `design_source/fit/registry.csv` exists (or reference screenshots are present). Record `clone: true|false` — it drives the per-screen Visual-QA task and the final Fit-gate phase below.
...
3. **Clone only — per screen this phase builds/edits:** add `- [ ] TASK-NN.k **Visual QA** — render <Sxx> and compare against its reference image <ref> per fit/<Sxx>.md; file any divergence`. This makes per-screen reference fit a done-criterion, not an afterthought.
...
**(e) State deltas.** Return (do not write) one PROGRESS "Phase completion" row per NEW phase (`| NN | <title> | <status> | — | generated; see PHASE_NN |`), one Decisions-log line (`<date> — Phase plan (re)generated by {{PREFIX}} --plan --phases from <source>. Cross-ref: 00_overview §1.`), and the matching `00_overview` §1/§2/§3 deltas. Never edit "Current state" prose or existing session-log lines.

**(f) Final Fit-gate phase — CLONE ONLY.** When `clone: true`, append a terminal phase
`PHASE_<last+1>_fit_gate` (prereqs = all screen-building phases). Its tasks:
1. `- [ ] TASK-NN.1 Run `/{{PREFIX}} --fit` over every screen in fit/registry.csv.`
2. `- [ ] TASK-NN.k **Fix divergences** — implement each filed divergence SPEC (`/{{PREFIX}} --feature --next`), re-run `--fit`.`
3. `- [ ] TASK-NN.k Update PROGRESS.md.`
Its **Done criteria**: `/{{PREFIX}} --fit` reports 0 unexplained divergences (only `spec/deviations.md` entries remain) and the overall fit score meets the project's clone-done threshold. This is the user-requested "reference-comparison as the last phase".

## Idempotency & merge (sync / re-runs)
- Wrap generated regions in HTML-comment sentinels: `<!-- {{PREFIX}}:plan:gen id=PHASE_NN hash=… generated=<date> -->` … `<!-- /{{PREFIX}}:plan:gen -->`. `## Notes for next session` is **human-owned — never written**.
- Checkbox merge keyed by `TASK-NN.k`: existing `- [x]` → preserve state AND wording (reword diff → `conflicts[]`, keep existing); existing `- [ ]` → safe to update text; new id → insert at deliverable position `<!-- {{PREFIX}}:plan:added <date> -->`; absent-but-checked → keep + `<!-- {{PREFIX}}:plan:orphan -->`; absent-and-unchecked → propose removal in preview (needs explicit `y`).
...
=== PLAN ===
{
  ...
  "phases": [{
    ...
    "tasks": [
      ...
      {"id":"TASK-08.9","text":"**Visual QA** — render S01 and compare against 05.png per fit/S01.md; file any divergence","layers_implied":["presentation"],"test_types_implied":[],"traces":{"fit":["S01"]}}
    ],
