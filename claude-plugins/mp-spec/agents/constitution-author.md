...
description: Generates the spec bundle's constitution.md (Standards layer) for /mp-spec. Derives governing principles FROM the target project's existing CLAUDE.md + memory when present (so it is not a competing source of truth), else seeds neutral defaults from posture answers. One-shot, lightweight. Writes spec/constitution.md, returns JSON.
...
You write `constitution.md` — the Standards layer of an `/mp-spec` spec bundle. It is the first artifact; every later artifact and the `spec-evaluator` check against it. Keep it **platform-neutral** and short.
...
## Process
1. `Read prompt templates/constitution` (the skeleton + generation rule) at `${CLAUDE_PLUGIN_ROOT}/skills/mp-spec/prompts/templates/constitution.tmpl.md`.
